...
#' 
#' # Generate Dirichlet responses using base R (no extra packages needed)
#' alpha <- c(2, 3, 4)
...
#' df_par <- DirichletForest_distributed(X, Y, B = 100, n_cores = 2)
#' 
#' # Example 3: Weight-based mode (stores samples for distributional analysis)
#' df_weights <- DirichletForest_distributed(X, Y, B = 100, store_samples = TRUE, n_cores = 2)
#' 
...
DirichletForest_distributed <- function(X, Y, B = 100, d_max = 10, n_min = 5, 
  ...
  if (.Platform$OS.type != "windows") {
    ...
    parallel::clusterApply(cl, seq_len(n_cores), function(worker_id) {
      ...
      assign("worker_forest", worker_forest, envir = .GlobalEnv) #nolintr

      assign("Y_train", Y, envir = .GlobalEnv) #nolintr
...
#' df <- DirichletForest_distributed(X, Y, B = 50, store_samples = TRUE, n_cores = 2)
#' 
#' # Create test data
#' X_test <- matrix(rnorm(10 * p), 10, p)
#' 
...
#'
#' @param forest_model A forest model created by \code{\link{DirichletForest_distributed}} with store_samples = TRUE
